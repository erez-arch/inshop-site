...
const T = {
  ...
  he: {
    ...
    nav_cta: "הצטרפו אלינו",
    hero_tag: "רשת קמעונאות אוטונומית",
    hero_h1a: "החנות", hero_h1b: "מכירה אותך.",
    hero_sub: "גש. תזוהה. צא.\nללא כרטיס. ללא אפליקציה. ללא תור.",
    hero_btn1: "ראה את זה בפעולה", hero_btn2: "הטכנולוגיה",
    marquee: ["זיהוי פנים","תשלום ללא מגע","אפס תור","גישה 24/7","מלאי חכם","מילוי מאומת AI","רישום חד פעמי"],
    machine_tag: "החומרה", machine_h2a: "יחידה אחת.", machine_h2b: "שש טכנולוגיות.",
    caps: [
      { label:"זיהוי", title:"זיהוי פנים", detail:"מנוע בינה מלאכותית פנימי מזהה כל לקוח תוך פחות משנייה. ללא כרטיס, ללא טלפון, ללא מגע. רישום חד-פעמי בכל יחידה — וממנה ואילך זוהי הגישה שלך לנצח.", tags:["פחות משנייה","ללא מכשיר","גישה לכל החיים"] },
      { label:"התאמה", title:"תצוגה חכמה", detail:"ברגע שהמצלמה מזהה אותך, המסך מציג את שמך, הצעות אישיות ומבצעים מותאמים. כל לקוח רואה מסך שנבנה עבורו בזמן אמת — לא אחד גנרי.", tags:["ברכה אישית","הצעות מותאמות","מבצעים ממוקדים"] },
      { label:"מלאי", title:"מלאי חי בזמן אמת", detail:"כל מוצר בכל מדף מנוהל בזמן אמת בכל היחידות. ברגע שרמת המלאי יורדת מתחת לסף שהוגדר — הזמנה יוצאת לספק אוטומטית. אין צורך שאיש יבדוק.", tags:["מעקב בזמן אמת","הזמנה אוטומטית","אפס חוסרים"] },
      { label:"אימות AI", title:"בדיקת מדף חכמה", detail:"לאחר כל מילוי, הממלא מצלם את המדף. מנוע ה-AI בודק מיקום, כיוון תווית וכמות מוצרים. התשלום מאושר רק כשכל פרמטר עובר את הבדיקה.", tags:["ניתוח תמונות","זיהוי תוויות","תשלום לאחר אישור"] },
      { label:"קופה", title:"תשלום עם סגירת הדלת", detail:"סגור את הדלת. זהו כל מה שנדרש. המערכת עקבה אחרי כל מה שלקחת, חייבה אוטומטית ושלחה קבלה ב-SMS. ללא סריקה, ללא תור, ללא שום מגע.", tags:["דלת = קופה","חיוב אוטומטי","קבלה ב-SMS"] },
      { label:"מערכת", title:"מערכת הפעלה INSHOP", detail:"ספקים, ממלאים, לקוחות וכל יחידה פעילה — מחוברים יחד למערכת הפעלה אחת. לוח בקרה אחד. מקור אמת אחד. ללא שיחות טלפון, ללא גיליונות אקסל, ללא ניחושים.", tags:["מחסנית מלאה","כל המיקומים","סנכרון מלא"] },
    ],
    exp_tag: "החוויה", exp_h2a: "שש שלבים.", exp_h2b: "מרגיש כמו כלום.",
    steps: [
      { title:"גש ליחידה", body:"גש לכל יחידת INSHOP. אין צורך בשום פעולה מוקדמת." },
      { title:"זיהוי פנים", body:"מנוע הבינה המלאכותית מזהה אותך תוך פחות משנייה. ללא כרטיס. ללא טלפון." },
      { title:"הדלת נפתחת", body:"הדלת נפתחת ברגע שהמערכת מזהה אותך. מיידי, ללא מגע." },
      { title:"קח את המוצרים שלך", body:"קנה בצורה טבעית. קח מה שצריך, עיין בחופשיות — ללא כל לחץ." },
      { title:"סגור את הדלת", body:"זו הקופה. המערכת עקבה אחרי כל מה שלקחת ועיבדה את התשלום." },
      { title:"קבלה ב-SMS", body:"התשלום מתבצע אוטומטית. קבלה מגיעה ב-SMS תוך שניות." },
    ],
    exp_cta: "רישום חד פעמי. קנייה בכל מקום. לנצח.",
    tech_tag: "מאחורי הקלעים", tech_h2a: "כל שכבה", tech_h2b: "מחוברת.",
    tech_sub: "INSHOP OS מנהל את כל הפעולה — חומרה, תוכנה, לוגיסטיקה ואנשים — במערכת אחת.",
    tech_cards: [
      { icon:"👁", title:"זיהוי פנים", body:"רישום חד-פעמי. מכאן ואילך, המערכת מזהה אותך ברגע ההגעה — ללא חיכוך, לנצח.", color:"#00A1E0" },
      ...
      { icon:"📦", title:"מלאי בזמן אמת", body:"כל מוצר בכל יחידה מנוהל בזמן אמת. ירידה מתחת לסף — הזמנה יוצאת לספק אוטומטית, ללא התערבות אנושית.", color:"#06AED5" },
      { icon:"🤖", title:"בדיקת מדף חכמה", body:"לאחר כל מילוי, הממלא מצלם את המדף. מנוע AI בודק מיקום ועמידה בסטנדרטים לפני שמאושר כל תשלום.", color:"#5C67F2" },
      { icon:"📲", title:"מסך אישי", body:"הטרמינל מציג את שמך, הצעות מותאמות ומבצעים ממוקדים — ברגע שהמצלמה מזהה אותך.", color:"#1589EE" },
      { icon:"🔗", title:"מערכת הפעלה אחת לכל.", body:"ספקים, מלאי, ממלאים, יחידות ולקוחות — הכל מחובר למערכת הפעלה מרכזית אחת, בזמן אמת.", color:"#0070D2" },
    ],
    mod_tag: "בינה מובנית", mod_h2a: "שלושה מודולים.", mod_h2b: "רשת אחת.",
    modules: [
      { href:"/modules/supplier", num:"01", icon:"📦", title:"מודול ספקים", tagline:"זהה. הזמן. ספק. מלא.", desc:"המערכת מזהה מחסור ומפעילה את כל שרשרת האספקה אוטומטית — מהזמנה ועד מילוי מדף.", cta:"חקור מודול", color:"#00C7B1" },
      { href:"/modules/fillers", num:"02", icon:"♻️", title:"מודול ממלאים", tagline:"התראה. קבל. מלא. אמת.", desc:"ממלאים סמוכים נשלחים כשהמלאי יורד. AI מאמת את המילוי לפני אישור תשלום. ללא קיצורים.", cta:"חקור מודול", color:"#1589EE" },
      { href:"/modules/referral", num:"03", icon:"🤝", title:"מודול הפנייה", tagline:"שתף. מימש. הרוויח.", desc:"מאמנים ומקדמים חולקים קופונים דיגיטליים ומרוויחים עמלה רק כשרכישה אמיתית מתבצעת בטרמינל.", cta:"חקור מודול", color:"#00A1E0" },
    ],
    live_tag: "כבר פרוס", live_h2a: "לא הדגמה.", live_h2b: "חי עכשיו.",
    live_desc: "10 יחידות פועלות במלואן ברחבי ישראל. לקוחות אמיתיים. רכישות אמיתיות. תוצאות מדידות — כל יום.",
    live_stat1: "10", live_label1: "מיקומים חיים",
    live_stat2: "₪20K+", live_label2: "חודשי / יחידה",
    vis_tag: "החזון", vis_h2a: "נבנה בישראל.", vis_h2b: "מיועד לעולם.",
    vis_sub: "מתחיל עם חדרי כושר. מתרחב ל-20,000+ מיקומים. ואז לכל מקום.",
    phases: [
      { phase:"שלב 01", title:"מובילים בישראל", stat:"800", label:"חדרי כושר יעד", desc:"בניית נוכחות חזקה ובסיס משתמשים רשומים בתעשיית הכושר הישראלית.", color:"#00C7B1" },
      { phase:"שלב 02", title:"התרחבות גלובלית", stat:"20K+", label:"חדרי כושר בעולם", desc:"שכפול המודל המוכח לרשתות כושר מובילות ברחבי העולם.", color:"#1589EE" },
      { phase:"שלב 03", title:"מעבר לחדרי הכושר", stat:"∞", label:"קמעונאות אוטונומית", desc:"מרחובות המסחר, דרך משרדים ועד תחנות תחבורה — החנות האוטונומית המלאה.", color:"#5C67F2" },
    ],
    cta_tag: "דברו איתנו", cta_h2a: "הבא INSHOP", cta_h2b: "למיקום שלך.",
    ...
    mod_links: [["ספקים","/modules/supplier"],["ממלאים","/modules/fillers"],["הפנייה","/modules/referral"]],
  },
};
